...
function getMaxServings(recipe, available) {
    ...
    for (let key in recipe){
        if (recipe[key] <= available[key]){
            const x = Math.floor(available[key]/recipe[key])
            if (!min){
                min = x
            } else if (x < min) {
                min = x
...
// function getMaxServings(recipe, available) {
//     return Object.keys(recipe).reduce((maxServings, ingredient) => {
//         // if (!available.hasOwnProperty(ingredient)) return 0;
//         // return Math.min(maxServings, Math.floor(available[ingredient] / recipe[ingredient]));
//         return Math.min(maxServings, Math.floor((available[ingredient] ? available[ingredient] : 0) / recipe[ingredient]));
//     }, Infinity);
// }
