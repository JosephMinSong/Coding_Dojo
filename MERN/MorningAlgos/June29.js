...
function compareVersionNumbers(v1, v2) {
    ...
    let numberV1 = v1.split('.')
    let numberV2 = v2.split('.')

    for (let i = 0; i< numberV1.length; i++){

        if (!numberV2[i]) return 1

        if (parseInt(numberV1[i]) > parseInt(numberV2[i])){
            return 1
        } else if (parseInt(numberV1[i]) < parseInt(numberV2[i])){
            return -1
        } else {
            continue
        }
