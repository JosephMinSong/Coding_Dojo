...
function merge(left = [], right = []) {
    // merging left and right array that are already sorted
    let result = []
    let i = 0;
    let j = 0;
    let longer = left.length >= right.length ? left.length : right.length

    while ( i < longer && j < longer ){
        if ( left[i] == undefined){
            result.push(right[j])
            j++
...
function mergeSort(numbers = []) {
    // keep splitting until numbers.length is 1 and then works recursively to build it backwards
