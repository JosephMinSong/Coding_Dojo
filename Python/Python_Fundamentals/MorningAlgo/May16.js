...
function binarySearch(sortedNums, searchNum) {
    let lower_bound = 0
    let upper_bound = sortedNums.length - 1

    while (lower_bound <= upper_bound){
        let middle_index = Math.ceil((upper_bound - lower_bound)/2) //floor doesn't work for some reason?

        if (sortedNums[middle_index] === searchNum){
            let count = 1
            let going_up = middle_index + 1
            let going_down = middle_index - 1
            while (sortedNums[going_up] === searchNum){
                count++
                going_up++
            }
            while (sortedNums[going_down] === searchNum){
                count++
                going_down--
            }
            return count
