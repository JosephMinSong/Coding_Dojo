...
isSumOfHalvesEqual() {
    ...
    let halfCount = Math.floor(this.size()/2);
    let i = 0;
    let firstSum = 0;
    let secondSum = 0;
    ...
    while(i < this.size()){
        ...
        let dequeuedItem = this.dequeue();
        
        if(i < halfCount){
            firstSum += dequeuedItem;
        } else {
            secondSum += dequeuedItem;
        }

        this.enqueue(dequeuedItem);
        i++;
    }
...
console.log(two1.stack1.items); // [2,3,4,,5,6]
console.log(two1.stack2.items); // []
